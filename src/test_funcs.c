...
void	move_image(t_fdf *const fdf)// t_transformed_map *const map)
{
	if (fdf->param.time_tracker > 0.1)
	{
		...
		{
			fdf->param.y_offset += ((fdf->img->width / 50) / fdf->param.zoom);
			fdf->img->instances[0].y += fdf->img->height / 50;
		}
		...
		{
			fdf->param.x_offset += ((fdf->img->width / 50) / fdf->param.zoom);
			fdf->img->instances[0].x += fdf->img->width / 50;
		}
